...
- PMが部下（サブエージェント）を呼び出すと、その部下も小さいキャラクターとして
  一時的に登場する
- **プロジェクトごとにフロアが分かれ、それぞれに指示ボックスがある**。宛先はその
  フロアの担当者だけに絞られるので、別のリポジトリの担当へ誤って投げてしまわない
...
- **このオフィスの管理外で動いている claude も「ロビー」に表示される**。手元の
  ターミナルで開いたセッションが、どのフォルダで何をしているか分かる（指示は不可）
...
- **プロジェクト** — 作業ディレクトリの登録。1人が複数を担当できる
...
- ロビーに出る管理外セッションは表示のみで、指示は送れません。設定を持たないため
  配席も人格の割り当てもできないからです
- 自動テストはありません
