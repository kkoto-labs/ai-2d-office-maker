...
- ブラウザのオフィス画面で、各キャラが「待機中・検討中・作業中・相談中・報告中」を
  リアルタイムでアニメーション表示
- **何をしているかで動きが変わる**。ファイルを書いているときは机に向かった後ろ姿で
  タイピングし、調べものをしているときは左右を見回す。こちらを向いているか机を
  向いているかで、待たれているのか作業中なのかが分かる
- PMが部下（サブエージェント）を呼び出すと、その部下も小さいキャラクターとして
  一時的に登場する
- **ワークスペースごとにフロアが分かれ、タブで行き来できる**。それぞれに指示ボックスが
  あり、宛先はそのフロアの担当者だけに絞られるので、別のリポジトリの担当へ誤って
  投げてしまわない。タブには稼働中の印と、見ていない間に報告が返った印が出るので、
  裏で走らせている作業の完了に気づける
- 同じキャラへの指示は同じ記憶（セッション）を引き継ぐ
- 「秘書」役が複数部署に横断で相談し、1つの統合報告としてまとめてくれる（部署間連携）。
  相談中は「築山創に相談中」のように相手が表示される
- **このオフィスの管理外で動いている claude も「ロビー」に表示される**。手元の
  ターミナルで開いたセッションが、どのフォルダで何をしているか分かる
- **ロビーのセッションはその場で社員として迎え入れられる**。部署と配属先の
  ワークスペースを選ぶだけで、それまでの会話の記憶を引き継いだまま組織に加わる
- 各キャラクターに`souls/*.md`で人格（口調・性格・クセ）を持たせられる

### 設定画面（画面右上の「⚙ 設定」）

- **エージェント** — 追加・削除、名前・部署・役職・キャラクター画像（127種）
- **サブエージェント** — 部下そのものの追加・削除と、説明・使えるツール・人格の編集
- **部署** — 新設と上下関係（社長直属か、別部署の下か）
- **ワークスペース** — 作業ディレクトリの登録。1人が複数を担当できる
- **組織図** — 関係性を図で確認し、ノードを選んでその場で付け外しできる
...
- `config/office.json`: 誰がいて・どんな関係で・どのワークスペースを担当するかの
...
- **ワークスペース**: 担当するディレクトリ。複数持てます。1つ目が作業ディレクトリ
...
**注意**: ワークスペースに追加したディレクトリは、`--permission-mode auto`で
...
- 同じキャラに同時に複数の指示を送ると、キューは1件ずつ順番に処理されます
  （状態ファイルへの書き込み自体はロックで直列化しています）
- 部署間の相談は、`claude`を子プロセスとして起動するBashコマンドで実現しています。
  Claude Codeには公式のセッション間メッセージング（`SendMessage`）がありますが、
  2.1.224以降かつmacOS/Linux限定のため、Windowsでは使えません。呼び出しは
  `server.py`の`consult_recipe()`1箇所に閉じてあるので、将来の差し替えは局所的に済みます
- ロビーに出るセッションへ、動いたまま指示を割り込ませることはできません。同じ会話に
  2つのプロセスが書き込むと壊れるためです。ターミナル側を終えてから「迎え入れる」と、
  会話の続きとして指示できます
- 自動テストはありません
