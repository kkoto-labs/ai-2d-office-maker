...
- PMが部下（サブエージェント）を呼び出すと、その部下も小さいキャラクターとして
  一時的に登場する
- ブラウザの指示ボックス、またはターミナル（`talk_to_*.sh`）のどちらからでも各キャラに
  指示できる。両方とも同じ記憶（セッション）を共有
...
- ブラウザの指示ボックスから、宛先を選んで指示を送れます
- ターミナルから直接話しかける場合は `./talk_to_p.sh`（企画部）、`./talk_to_d.sh`
  （開発部）、`./talk_to_m.sh`（広報マーケ部）、`./talk_to_s.sh`（秘書室）を使います
...
- `server.py`: ブラウザからの指示を受け取り、該当キャラのセッション（`state/sessions/`）
  を`claude -p --resume`で継続実行するローカルサーバーです。同じセッションIDを
  `talk_to_*.sh`からも使うので、ブラウザ・ターミナルどちらから話しかけても記憶が
  つながります（※開いている間のリアルタイム同期はしません。開き直すと最新の記憶に
  追いつきます）。
...
6. `talk_to.sh`の`case`文に追記し、`talk_to_<内部ID>.sh`を作成
7. `viewer/app.js`の`AGENTS`配列に追記
...
- ブラウザとターミナル（`talk_to_*.sh`）を同時に同じキャラへ使うと、状態ファイルへの
  書き込みが競合する可能性があります（ロック処理はしていますが、同時利用は避けてください）
- 開いている間のリアルタイム同期はしません（履歴は共有されますが、セッションを
  開き直すまで反映されません）
